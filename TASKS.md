...
- [x] Add `POST /local-tracks/{id}/rescue` endpoint: validate a final link exists, call `rescue_metadata`, return updated track record
- [x] Return 409 if no final link exists for the given local track
- [x] Write tests: tags are correctly written, APIC frame is embedded, 409 is returned when no final link exists
