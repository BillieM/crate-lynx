...
- [ ] Write SQLAlchemy base model and shared `models.py` (to be mounted/imported by `app/`)
- [ ] Define `local_tracks` table (id, file_path, library_root_rel_path, fingerprint, beets_id, created_at, updated_at)
- [ ] Define `streaming_accounts` table (id, provider, display_name, auth_token_blob, created_at, updated_at)
